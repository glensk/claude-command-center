...
- `ccc switch-account <label>` (`-s/--session`, `-u/--undo`, `-f/--force`, `-p/--prompt`, `-C/--no-continue`, `-K/--continue-if-interrupted`, `-q/--quiet`) — move THIS live session to another configured Claude account without losing the conversation: arms a one-shot relaunch that the release-locks Stop hook claims when the turn ends (same-event hooks run in PARALLEL, so that claim is never ordered after your auto-commit); the detached `ccc switch-now` then waits for the Stop chain to be OBSERVED drained — refusing the kill (exit 1, tab alive) on a timeout, an unreadable `ps` or an identity change — terminates the bound Claude process, waits for the tab's tty to be back at a POSIX shell prompt and types `cd <cwd> && ( <account env pin> claude --resume <id> )` into that same iTerm tab / tmux pane. `-p/--prompt [TEXT]` appends Claude Code's positional prompt to that line (bare `-p` sends `continue`), so the relaunched session submits a turn on the new seat by ITSELF instead of parking at an idle composer — a plain resume keeps the conversation but nobody drives it, which is the whole reason the switch was made; `-C/--no-continue` suppresses it whatever the flag order, and a prompt holding a control character is refused (the line is TYPED into a shell, where a newline submits early). `-K/--continue-if-interrupted` decides that auto-continue from the session's STATE instead of hard-coding it, so a seat change preserves what the session was doing: `continue` is submitted only when its work was really interrupted — its last main-chain turn died on THIS account's usage limit (`limitswitch.observe`, which unlike `is_halted` also rejects a transient server-side 429: "not your usage limit" means the other seat hits the same overloaded server), or `-f` cut a live turn off — and an idle session relaunches idle, waiting at its composer exactly as it was left. An explicit `-p` (any text) or `-C` answers the question outright and the gate never runs; an unreadable transcript answers "not interrupted" (the recoverable side: the user types one word, versus unattended work nobody asked for). The `-N` outcome line then ends with "It lands IDLE: …" so a silent composer reads as the deliberate choice it is. This is what the shipped `/cpriv-to-cwork` and `/cwork-to-cpriv` send (2026-09-09: a long-idle session moved to the other seat used to relaunch and start working); the automatic failover is unaffected — a turn that dies on a rate limit is interrupted work by definition and `limitswitch` passes its own explicit prompt. The armed path carries it in the row's `switch_prompt`, claimed and spent with the arm; `-N` hands it to `switch-now --prompt` directly. Fails closed (exit 1, nothing armed) when: only one account is configured, the label is unknown or already the billing account, the shell is not the named session (`CLAUDE_CODE_SESSION_ID` / `CLAUDE_PID`), the id is live under two accounts, background tasks/agents are still in flight (process tree + the transcript's launch/notification records + the `SubagentStart`/`SubagentStop` counter, which is the only evidence of an in-process Agent-tool subagent — it spawns no child process and its transcript record can lag; `--force` overrides), the target dir is not logged in as its `claude_account_emails` identity, the target cannot see the transcript at `<dir>/projects/<cwd slug>/<id>.jsonl` (share the store: symlink `projects/`), or the cwd could not be confirmed trusted for the target. `switch-now` re-checks identity/trust/same-file transcript before the kill and again after the exit; any abort is logged to `events.log` (`switch-now`) and raised as a desktop notification carrying the manual command. Wire it to a slash command per direction (e.g. `/to-work` → `ccc switch-account work -s "$CLAUDE_CODE_SESSION_ID"`, then end the turn). `-N/--now` skips the arm and relaunches immediately — the way out of an account that hit its session limit (its prompts, slash commands included, are rejected before any turn, so nothing after-turn can fire): run it with zero model quota from a slash command whose file carries the inline expansion `` !`ccc switch-account <label> -s "$CLAUDE_CODE_SESSION_ID" -N -c -K` `` (inline expansions execute client-side BEFORE any model request — unlike the interactive `!` prefix, whose output Claude answers as a normal turn; `-c/--cancel-prompt` exits 1 after spawning the relauncher so Claude Code cancels the prompt outright, and the registry's "busy" raised by that very prompt is not treated as a turn in flight) or `ccc switch-account <label> -s <id> -N` from any other terminal; the session must be idle (`-f` cuts a turn off). `$ARGUMENTS` is substituted inside the expansion, so `… -N -c -K $ARGUMENTS` lets `/to-work -f` force past the in-flight veto — and `/to-work -p` force the auto-continue, `/to-work -p "<text>"` send a different first prompt, or `/to-work -C` forbid one — without a second command file (the shipped `/cwork-to-cpriv` and `/cpriv-to-cwork` are exactly this). The veto's transcript evidence credits a `<task-notification>` in every shape Claude Code has written one — an `attachment` record (≤ 2.0), a `user` record whose content is the notice (current), or a `queue-operation` holding one queued mid-turn — a scanner knowing only the first reported 28 finished tasks as in flight.
...
- `ccc codex-switch` (`-t/--target`, `-n/--dry-run`, `-p/--pid`, `-i/--input`) · `ccc codex-switch-now` · `ccc codex-switch-exec <token> [-S]` — the Codex twin of `switch-account -N`: move a LIVE Codex TUI thread to another ChatGPT login (`CODEX_HOME`) in the same iTerm tab / tmux pane, zero model tokens, so it works after "Usage limit reached". Codex has no account beyond `$CODEX_HOME/auth.json`, so the move is: quit the TUI, port the thread's rollout into the target home, relaunch `codex … resume <thread>` there. The trigger is a `UserPromptSubmit` hook (client-side, runs BEFORE any request; its `{"decision":"block"}` cancels the prompt) — `$CODEX_HOME/hooks/codex-switch-hook.py` pipes the payload into `ccc codex-switch`, registered once in the shared `config.toml` with a `hooks.state` trust row per seat (the key embeds each seat's own config path; the hash is `sha256:` of the canonical JSON of the normalized handler, `codex-rs/hooks/src/engine/discovery.rs::hook_hash`). Type `/switch [seat[!]]` WITH a leading space (the composer rejects an unknown slash command; a leading space or a `/` inside the name bypasses that check), or `//switch`, `/switch/`, `switch`. The seat is a label (`default`/`private`/an extra), a `codex_seat_aliases` alias (`["work=default", "gl=private"]`) or the login e-mail; bare `/switch` takes the seat oracle's best OTHER seat (`codex_homes_in_order(probe=False)`: cooldowns, refusals, 100 % windows, pins, fill policy and the weekly `codex_seat_rota` all honoured — a seat whose week belongs to a colleague is never the automatic pick); an explicit seat the oracle does not list is refused unless spelled `<seat>!`, and the forced switch's note then names the block it overrode (`forced past ccc's seat oracle on 'default' (rota: 14.9.–20.9. used by alice)`). Planning (hook side) fails closed with the reason as the block: the `transcript_path` must be a real rollout under a registered seat whose `session_meta` names the thread; the codex process is the OUTERMOST exact-name `codex` ancestor, foreground on its tty, with a `CODEX_HOME` agreeing with the transcript's home; the target must be logged in, not hold the thread's writer lock (`flock` probe on `thread-writer-locks/<uuid>.lock`), its SQLite row (if any) must name the mirrored path, a `-p/--profile` must exist identically as `<home>/<name>.config.toml` there; `--remote` sessions and unknown codex options refuse (complete allow-list of 0.153.4's TUI options, before and after `resume`, `=`/attached-short spellings included; `-i`, positionals and `resume`'s selector flags are dropped). The launch is a 0600 record under `<app_home>/codex-switch/<token>.json` (exe, options, carried env — the launcher families `AI_* TP_* CCC_* CLAUDE_SESSION_* CODEX_*` minus Codex's storage-routing names and secret-shaped names; values never appear in reasons, logs or argv). `codex-switch-now` (detached) waits for the hook process to exit, re-binds the pid to the tab, types `/quit` (never a signal — the TUI has no SIGTERM handler), waits for the process to be gone, the tty to sit at a shell prompt twice a second apart and the source writer lock to be released, then ports the rollout by PREFIX-MERGE (the target's existing copy must be a prefix after nulling `rate_limits` on both sides; its own bytes are kept so its seat telemetry and Codex's byte-offset history projection stay intact, only the new tail is scrubbed; a longer or differing copy is set aside as `*.switch-diverged-<ts>` and the switch refused) and types `ccc codex-switch-exec <token>`, which `execve`s codex under the target `CODEX_HOME`. Any failure after the source exited types `… -S` instead (the source seat comes back — the tab never ends dead); before it, the TUI is left alive. Everything lands in `events.log` (`codex-switch`) and, on abort, a desktop notification with the manual command. A `<thread>.pending` marker (120 s) refuses a second `/switch` while one is in flight. The relaunched session lands IDLE. Tests: `tests/test_codex_switch.py`.
...
it" — it runs `ccc mark-done --close -q`, which marks the session done and then, once the
turn's Stop-hook chain has been OBSERVED drained, SIGTERMs the Claude process and closes
its terminal pane/tab. The wiring is the internal `ccc close-now --session <id> [--pid
<CLAUDE_PID>] [--iterm <id>]` command, spawned by the `release-locks` Stop hook once the
close request is atomically claimed. Same-event hooks run in PARALLEL, so `close-now`
cannot be ordered after a foreign auto-commit: it polls the process tree instead (two
...
- **Stop lease, not a Stop floor** — Claude Code runs every hook of one event in PARALLEL, so
  `release-locks` cannot be ordered after your auto-commit. At end of turn it therefore LEASES the
  session's locks (`protected_until = now + stop_barrier_wait_sec`) instead of dropping them: a
  leased row denies every peer whatever the holder's liveness or TTL, tells the denied session its
  files are being committed and in how many seconds the lock frees, and frees itself when the
  stamp expires. The holder's next
  edit of a file clears that file's lease (a new turn). `ccc handoff <file>` is the way to hand a
  file over immediately without waiting the lease out; `ccc lock-release [--all]` and deleting a
  session row (`ON DELETE CASCADE`) drop leased rows by design. See
  [hooks.md](hooks.md#the-stop-hook-lock-lease).
- **Fail-open** — any error in the lock path (store down, adapter error) lets the edit through; a
  *deny* only ever happens when a peer is provably live and the lock fresh, or the lock leased.
  Kill-switch: `file_lock_enabled = false`.

| setting                   | default | what it does                                                                                                                                                                                                                                                      |
| :------------------------ | ------: | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `file_lock_enabled`       |  `true` | the cross-session file lock itself (kill-switch)                                                                                                                                                                                                                  |
| `file_lock_ttl_sec`       |  `1800` | a held lock with no edit for this long is stale → reclaimable                                                                                                                                                                                                     |
| `file_lock_wait_sec`      |     `0` | `>0`: `PreToolUse` polls a held lock this long before denying                                                                                                                                                                                                     |
| `stop_barrier_enabled`    |  `true` | end of turn LEASES the session's locks instead of releasing them (off = the pre-lease release)                                                                                                                                                                    |
| `stop_barrier_wait_sec`   |   `180` | how long that lease protects them, and how long `close-now`/`switch-now` wait for the Stop chain (clamped to `0..900`; `0` is NOT an opt-out — it empties the lease and makes every close/relaunch unprovable, hence refused: use `stop_barrier_enabled = false`) |
| `stop_barrier_settle_sec` |     `2` | required gap between the two clean drain scans before a `close-now`/`switch-now` kill                                                                                                                                                                             |
...
*A login you SHARE with a colleague.* `codex_seat_rota` puts one seat on a weekly rota —
one `"label=YYYY-MM-DD@IANA_ZONE:name,name[,name…]"` entry per shared seat, where the date
is the **Monday** of the FIRST name's week and the names take turns from there, forever
(forwards and backwards from that Monday). Weeks are `[Monday 00:00, next Monday 00:00)` in
the entry's OWN zone — the zone is required, so travelling or another consumer's `$TZ`
cannot move the day the seat changes hands. `codex_seat_rota_me` says which of the names
this machine is; membership is decided per seat. During somebody else's week the seat is a
BLOCKED quota row (`blocked_by="rota"`, `resets_at` = your next Monday, or the underlying
block's own reset when that is later) for EVERY Codex consumer — ranking, pin, probe,
`headroom`, `-Q`, a registered `$CODEX_HOME`, a journal resume — and only a human
`/switch <seat>!` overrides it. An entry ccc cannot read, or one whose names do not contain
`codex_seat_rota_me`, blocks that seat too (failing open would bill a colleague's week); an
entry naming no configured seat is reported in `ccc quota -j`'s `codex_seat_rota_errors` and
ignored. Manage it with `codex-in-claude rota [show|set|clear|me]` (`-s` the Monday, `-z` the
zone, defaulting to this machine's); full reference in
[docs/codex.md](codex.md) § "Sharing a seat on a weekly rota".
...
or an explicit `-c` removes it. An `observed` entry whose scope is exactly `quota` is
SUPERSEDED (without being deleted) by the seat's own newer, unblocked, well-formed and
`available` usage reading — the 2026-09-14 fix for a three-day-old refusal that outlived
every measurement contradicting it; a hold and the auth/entitlement scopes are never
superseded. `scope` tags why a block exists — `ai.py` records
