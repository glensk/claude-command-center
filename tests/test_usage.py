...
def test_abbrev_email_squeezes_the_local_part_and_never_the_domain() -> None:
    ...
    # two logins of the same person apart (``an.gr.de`` vs ``an.gr``), which
    # ``andrea…`` would hide.
    assert usage.abbrev_email("andrea.gray.de@example.org", squeeze_local=True) == (
        "an.gr.de@example.org"
    )
